...
/**
 * مسارات النماذج.
 * - وضع msforms: يستقبل الخادم الجزء غير الشخصي فقط (الكتاب، التخصص، التواريخ)،
 *   وتُكمل الطالبة بياناتها في Microsoft Forms التابع للمؤسسة مباشرة.
 * - وضع flow: يمرر الخادم النموذج الكامل إلى Power Automate دون تخزينه،
 *   ويحفظ الجزء غير الشخصي فقط للإحصائيات.
 */
export const formsRouter = Router();

formsRouter.post('/borrow/request', (req, res) => {
  ...
  const book = booksRepo.get(data.bookId)!;
...
formsRouter.post(
  ...
  wrap(async (req, res) => {
    ...
    try {
      ...
      if (result.channel === 'sink') loansRepo.update(loan.id, { channel: 'sink' });
      trackEvent({ type: 'borrow_submitted', targetId: book.id });
      ...
      loansRepo.remove(loan.id); // لا نُبقي سجلًا لطلب لم يصل للمؤسسة
      throw e;
    }
  }),
);

formsRouter.post('/visit/anonymous', (req, res) => {
