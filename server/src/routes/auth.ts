...
authRouter.get('/setup-status', async (_req, res) => {
  res.json({ hasUsers: (await usersRepo.count()) > 0 });
});
...
authRouter.post('/change-password', requireAuth, async (req, res) => {
  ...
  const updated = await usersRepo.update(user.id, { passwordHash: await hashPassword(newPassword) });
  if (!updated) throw new HttpError(404, 'المستخدم غير موجود');
