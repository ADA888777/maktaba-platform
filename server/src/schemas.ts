...
/** http(s) link, an uploaded file inside the platform, or empty */
...
/** The specialty is optional: the borrow form only asks for the trainee name, trainee id and the book */
const specialty = text(80).default('');

const returnAfterBorrow = <T extends { borrowDate: string; returnDate?: string | null }>(v: T) =>
  !v.returnDate || v.returnDate >= v.borrowDate;

/** The non personal part of a borrow request - the only part that reaches the site database */
...
/** Trainee id: five to fifteen digits */
export const traineeId = z.string().trim().regex(/^\d{5,15}$/, 'الرقم التدريبي يجب أن يتكون من 5 إلى 15 رقمًا');
...
/** The anonymous part of the library visit survey (for statistics) */
export const visitAnonymousSchema = visitBase.superRefine(visitRules);

/** The full form when it is sent through Power Automate */
