...
const cfg = config?.visit;
  ...
  // the specialty list is managed from the dashboard settings: the field only appears once it is filled
...
specialty: specialties.length ? rules.required(v.specialty, 'التخصص') : '',
...
setDone({ url, embed: cfg.openIn === 'embed' });
  ...
  <Spinner />
    ...
    سُجّلت إجاباتك في الإحصائيات. <strong>الخطوة الأخيرة:</strong>strong> أرسلي النموذج الرسمي للمؤسسة لإكمال المشاركة.
    ...
    <h2 className="mt-4 text-2xl font-bold">شكرًا لمشاركتك!</h2>h2>
    <p className="mt-2 text-ink-600">تم تسجيل إجاباتك بنجاح، وستساعدنا في تحسين خدمات المكتبة.</p>p>
    <div className="mt-6 flex flex-wrap justify-center gap-3">
    <ButtonLink to="/" variant="primary">العودة للرئيسية</ButtonLink>ButtonLink>
    <Button variant="outline" onClick={() => setDone(null)}>مشاركة جديدة</Button>Button>
    </div>div>
    </div>div>
    </div>div>
    )
  ...
  <Input label="الاسم" name="fullName" required autoComplete="name" value={values.fullName} onChange={(e) => set('fullName', e.target.value)} error={errors.fullName} wrapperClass={specialties.length ? undefined : 'sm:col-span-2'} />
